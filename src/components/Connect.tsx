...
import { repr, sub } from '../util/point'
...
export const Connect = React.memo(function (props: { a: Point; b: Point }) {
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} style={{ position: 'absolute', zIndex: -1, top, left, width, height }}><path d={`M${repr(sub(a, topLeft))} C${repr(c1)} ${repr(c2)} ${repr(sub(b, topLeft))}`} fill="transparent" stroke="black"></path></svg>
})
